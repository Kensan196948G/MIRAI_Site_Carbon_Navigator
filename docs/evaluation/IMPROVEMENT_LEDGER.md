...
| # | 課題 | 影響度 | 必要操作 |
|---|---|---|---|
| 1 | 公開 URL が 530（Tunnel 断）継続中 | 重大 | Cloudflare ダッシュボードで Tunnel/トンネルトークンを復旧・ローテーション |
| 2 | 本番 DB の既定アカウント無効化 | 重大 | 承認後 `scripts/disable_default_users.py --deactivate` 実行 |
| 3 | SMTP/Teams 通知未設定 | 高 | M365/Teams 連携の設定と通知試験 |
| 4 | 実地 PoC（2現場）未実施 | 高 | Phase 8 計画の実行 |
| 5 | 排出係数の一次情報突合（電力・材料・建機等） | 高 | 環境省公表値等との照合・更新 |
